...
If you install the `recording-studio-device-sessions` addon gem, disable the corresponding built-in feature to avoid overlap:
...
automatically enable capability behavior (`commentable`, etc.).
...
A capability is enabled for a recordable type only when that model includes its mixin. For example,
using the `commentable` capability:
...
- `Page` supports the capability.
- Other recordable types are **not** opted in unless they also include the mixin.
- Installing an addon gem does not silently enable behavior globally.

Mixins may come from addon gems or from `RecordingStudio::Capabilities::*` for any remaining built-ins.
...
Capability mixins can accept parameters; they are not only on/off flags. For example:
...
Capability calls outside configured rules raise errors.
