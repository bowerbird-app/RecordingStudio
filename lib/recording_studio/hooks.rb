...
module RecordingStudio
  ...
  #     Rails.logger.info "RecordingStudio initialized!"
  #   end
  #
  ...
  class Hooks
    ...
    def log_hook_error(error, event_name, _hook)
      ...
      Rails.logger.error "[RecordingStudio::Hooks] Error in #{event_name} hook: #{error.message}"
    ...
    class << self
      ...
      # @return [Array] Results from all hooks
      def run(event_name, *)
        RecordingStudio.configuration.hooks.run(event_name, *)
      end
      ...
      def run_around(event_name, context, &)
        RecordingStudio.configuration.hooks.run_around(event_name, context, &)
      end
