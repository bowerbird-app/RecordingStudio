...
module ControlRoom
  ...
  #   ControlRoom.configuration.hooks.after_initialize do
  #     Rails.logger.info "ControlRoom initialized!"
  #   end
  #
  ...
  class Hooks
    ...
    def log_hook_error(error, event_name, _hook)
      ...
      Rails.logger.error "[ControlRoom::Hooks] Error in #{event_name} hook: #{error.message}"
    ...
    class << self
      ...
      # @return [Array] Results from all hooks
      def run(event_name, *)
        ControlRoom.configuration.hooks.run(event_name, *)
      end
      ...
      def run_around(event_name, context, &)
        ControlRoom.configuration.hooks.run_around(event_name, context, &)
      end
