  ...
  module Services
    ...
    class BaseService
      ...
      def run_before_hooks
        ...
        ControlRoom::Hooks.run(:before_service, self.class, service_args)
      end
      ...
      def run_after_hooks(result)
        ...
        ControlRoom::Hooks.run(:after_service, self.class, result)
      end
      ...
      def run_with_around_hooks
        if hooks_enabled? && ControlRoom.configuration.hooks.registered?(:around_service)
          ControlRoom::Hooks.run_around(:around_service, self) { perform }
        else
          perform
        end
      end

      # Check if hooks are enabled (ControlRoom is loaded and configured)
      #
      # @return [Boolean]
      def hooks_enabled?
        defined?(ControlRoom) &&
          ControlRoom.respond_to?(:configuration) &&
          ControlRoom.configuration.respond_to?(:hooks)
      end
