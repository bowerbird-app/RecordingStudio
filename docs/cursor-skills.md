...
- `.cursor/install.sh` provisions Ruby, PostgreSQL, dependencies, the dummy
  ...
  provision when Ruby, bundle, and Postgres are already usable. It strips a
  `ruby-` prefix from `.ruby-version` so the version matches MRI
  `RUBY_VERSION`. A skippable provision failure does not fail the Build. It
  always runs `.cursor/fetch-skills.sh` last.
